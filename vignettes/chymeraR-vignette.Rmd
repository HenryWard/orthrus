...
The first thing we want to do for any experiment is to generate quality control (QC) plots for raw read count data in order to diagnose fundamental issues with screens. Specifically, we want to make the following plots:

* Histograms of log-scaled read counts for each replicate.
* Total reads for all screens, relative to expected coverage for each replicate.
* A heatmap of Pearson correlations between log-scaled read counts for all replicates.

A single function in chymeraR, `plot_reads_qc`, takes care of this for all screens. Like other plotting functions that output multiple plots in chymeraR, `plot_reads_qc` requires you to specify a folder to save plots to. Because these are QC plots, we will output them to the previously-created QC folder. 
...
The final set of QC plots that we need to make consists of scatterplot comparisons between each pair of technical replicates. To make these, we call `plot_lfc_qc`. While it is possible and sometimes could be a good idea to make these before computing LFCs, these results tend to be more consistent after computing LFCs due to depth-normalization, so we recommend making them at this point in the pipeline. Although this function also outputs Pearson correlations between all technical replicates, like all QC plots, manual inspection for potential problematic outlier guides or other red flags or is recommended.

```{r}
plot_lfc_qc(df, screens, qc_folder)
```

Additionally, for many libraries we also want to measure how well guides that target essential genes dropout compared to all other guides. To compute the AUC under the ROC curve for all essential-targeting guides, we call the function `essential_lfc_qc`, which outputs results to the file "essential_PR_QC.txt" in the given QC folder. AUC values closer to 1 are typically better. To call this function, we also have to pass in the names of columns containing symbols for each gene.

```{r}
essential_lfc_qc(df, screens, "Gene.symbol1", "Gene.symbol2", qc_folder)
```

Before we can parse the data into a structure more suitable for scoring, we need to ensure that we differentiate gene pairs for intergenic-targeting guides from gene pairs where both guides target the same gene. Not all libraries will require this step, but for our library it's necessary. Intergenic regions are wrongly denoted for exonic-exonic dual-targeting guides by the symbol "---", which we will change to "None".
...
By default the chosen test is moderated-t with loess correction on residuals enabled and downstream Benjamini and Yekutieli FDR correction. Although chymeraR supports other types of testing, which are documented in the package, because they are less sensitive we do not recommend them at this time. 

The scoring function outputs a named list containing two elements. The first element, `scored_data`, is a dataframe of scored data for all gene pairs. The second element, `residuals`, is a dataframe of per-guide effects and residual values for all guides. The scored data is used for hit-calling through the function `call_significant_response`, as well as final analyses, and the residuals are used to make guide-level plots of 

``` {r}
# Scores data
temp <- score_conditions_vs_control(dual, screens, "HAP1_T12", "Torin_T12", 
...
```{r}
plot_lfc(dual_scores1, residuals, "HAP1_T12", "Torin_T12", file.path(lfc_folder, "dual_lfc_t12"),
         neg_type = "Sensitizer", pos_type = "Suppressor")
plot_lfc(dual_scores2, residuals, "HAP1_T18", "Torin_T18", file.path(lfc_folder, "dual_lfc_t18"),
         neg_type = "Sensitizer", pos_type = "Suppressor")
```
...
```{r}
# Plots drug response
p <- plot_significant_response(dual_scores1, "HAP1_T12", "Torin_T12", neg_type = "Sensitizer", pos_type = "Suppressor")
ggsave(file.path(output_folder, "torin_vs_hap1_t12.png"), width = 10, height = 7, dpi = 300)
p <- plot_significant_response(dual_scores2, "HAP1_T18", "Torin_T18", neg_type = "Sensitizer", pos_type = "Suppressor")
...
```{r}
p <- plot_significant_response_combn(paralog_scores, "HAP1_T12", loess = TRUE, neg_type = "Sensitizer", pos_type = "Suppressor")
...
p <- plot_significant_response_combn(paralog_scores, "RPE1_T18", loess = TRUE, neg_type = "Sensitizer", pos_type = "Suppressor")
ggsave(file.path(output_folder, "paralog_rpe1_t18.png"), width = 10, height = 7, dpi = 300)
p <- plot_significant_response_combn(paralog_scores, "RPE1_T24", loess = TRUE, neg_type = "Sensitizer", pos_type = "Suppressor")
...
```{r}
p <- plot_significant_response_combn(paralog_scores, "Torin_T12", filter_name = "HAP1_T12", loess = TRUE, neg_type = "Sensitizer", pos_type = "Suppressor")
...
ChymeraR also provides a way to score the effect of single-targeted genes (for exonic-intergenic guides that also comprise the null model for combinatorial scoring). Conceptually, this scoring method is the same as the condition vs. control comparison for dual-targeted guides, but is run separately on each orientation. In other words, for our example dataset we can also score conditions vs. controls for Cas9 guides separately from Cas12a guides. The interface to this the same as for dual-targeted scoring, but we provide an additional flag to tell ChymeraR to score orientations separately: `orientation=TRUE`. We also pass in the single-targeting guides and not the dual-targeting guides.

The main difference between the output of this call to `score_conditions_vs_control` and the previous ones is the function now returns a list of scored dataframes, one for each orientation. In this dataset, the first element of that list, `single_scores[[1]]`, contains Cas9 effects and the second, `single_scores[[2]]`, contains Cas12a effects. 
